// Might not need to use any of this because of Belt
...
//   open Option
//   \"<*>"(\"<*>"(Some(Util.eq), get(k, d0)), get(k, d1)) |> default(false)
// }
...
// let invert = d =>
//   List.fold_left(
//     (acc, (k, v)) => set(v, \"<$>"(get(v, acc), RList.append(k)) |> Option.default(list{k}), acc),
//     list{},
//     d,
//   )
...
//   List.map(k => {
//     open Option
//     (k, \"<*>"(\"<*>"(Some(f(k)), get(k, d0)), get(k, d1)) |> toExn("Error merging Dicts"))
//   }, intersect)
// }
...
// let where = (predD, d) => List.fold_left((acc, (k, f)) =>
//     acc
//       ? {
//           open Option
//           \"<*>"(Some(f), get(k, d)) |> default(false)
//         }
//       : false
//   , true, predD)
...
//     switch list {
//     | list{} => (List.rev(xs), List.rev(ys))
//     | list{(x, y), ...rest} => loop(rest, list{x, ...xs}, list{y, ...ys})
//     }
//   loop(d, list{}, list{})
// }
